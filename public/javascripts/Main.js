...
function drawAll(){
    for(let i = 0 ; i < rects.length;i++){
        ...
        yesRectSVG[i].updateWithOnClick(rects[i].x, rects[i].y + heightSize/3, heightSize/3, widthSize/3, 'red', '1', "populateRec(1)")
        noRectSVG[i].updateWithOnClick(rects[i].x + rects[i].width - widthSize/3, rects[i].y + heightSize/3, heightSize/3, widthSize/3, 'blue', '1', "populateRec(0)")
    }
}
function draw(index){

    rectsSVG[index].update(rects[index].x, rects[index].y, rects[index].height, rects[index].width, "#FFFFFF", '1')
    titleTextSVG[index].update(rects[index].x+widthSize/2, rects[index].y+50, '#444444', rects[index].title)
    yesRectSVG[index].updateWithOnClick(rects[index].x, rects[index].y + heightSize/3, heightSize/3, widthSize/3, 'red', '1', "populateRec(1)")
    noRectSVG[index].updateWithOnClick(rects[index].x + rects[index].width - widthSize/3, rects[index].y + heightSize/3, heightSize/3, widthSize/3, 'blue', '1', "populateRec(0)")
}
...
function populateRec(answer){
    ...
    if(rects[rects.length-1].x + $('#mainPanel').width() > $('#svg').width()){
        ...
        newYesRectSVG.updateWithOnClick(newRect.x, newRect.y, 25, 25, 'red', '1', "populateRec(1)")
        newNoRectSVG.updateWithOnClick(newRect.x + newRect.width - 25, newRect.y, 25, 25, 'blue', '1', "populateRec(0)")
        rects.push(newRect)
